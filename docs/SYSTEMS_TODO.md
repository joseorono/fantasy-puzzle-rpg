...
- [x] Battle system: Match-3 RPG combat with active hero skills, item bar, turn flow, and rewards. (Jose)
  ...
  - [x] Consumable battle item bar: party SPD-scaled shared cooldown. (Jose)
...
- [x] Skill System: Active skills (Ultimates) and Passive skills with 4 tiers per class, Indigolay icon sprite sheets, and resource costs. (Jose)
- [x] Pause Menu & Options: Full overlay with Items, Equip, Skills, Stats, Save, Load, and Options (Audio volume controls + mute) tabs with full keyboard navigation. (Jose)
- [x] Dungeon System: Multi-floor runs with authored/procedural generation, escalating encounters, floor exploration, rewards, and dungeon progress tracking. (Jose)
- [x] Save data: 3 manual slots + autosave, one localStorage key each, Zod-validated with a version/migration hook, save status indicator badge, and unload warning. See `docs/SAVE_LOAD_SYSTEM.md`. (Jose)
