...
export function calculateGuardChargeRate(party: CharacterData[]): number {
  const livingSpd = party.reduce((total, char) => (char.currentHp > 0 ? total + char.stats.spd : total), 0);
...
export function calculateGuardDecayResistance(party: CharacterData[]): number {
  const livingVit = party.reduce((total, char) => (char.currentHp > 0 ? total + char.stats.vit : total), 0);
 ...
 */
export function resolveGuardedDamage(incoming: number, guard: number, guardBreak: number = 1): GuardResolution {
