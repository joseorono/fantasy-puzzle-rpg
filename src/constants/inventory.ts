/*
 * Inventory and items constants
 */

import type { ConsumableItemData, EquipmentItemData } from '~/types';
import { CRAFTING_FEE } from '~/constants/blacksmith';

export const MAX_AMOUNT_PER_ITEM = 99;

/*
 * Items
 */
export const EquipmentItems: EquipmentItemData[] = [
  // --- Iron Tier ---
  {
    id: 'iron-sword',
    name: 'Iron Sword',
    type: 'equipment',
    description: 'A trusty, if a bit dull, sword.',
    pow: 5,
    vit: 2,
    spd: 0,
    forClass: 'warrior',
    comboBonus: 0.01,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 3, silver: 0, iron: 2 },
    iconName: 'steelSword',
  },
  {
    id: 'iron-short-bow',
    name: 'Iron Short Bow',
    type: 'equipment',
    description: 'A light bow, easy to handle for any novice archer.',
    pow: 3,
    vit: 0,
    spd: 3,
    forClass: 'rogue',
    comboBonus: 0.015,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 2, silver: 0, iron: 2 },
    iconName: 'leafBow',
  },
  {
    id: 'iron-staff',
    name: 'Iron Staff',
    type: 'equipment',
    description: 'A heavy stick for hitting things and occasionally casting spells.',
    pow: 4,
    vit: 1,
    spd: 1,
    forClass: 'mage',
    comboBonus: 0.01,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 3, silver: 0, iron: 3 },
    iconName: 'frostAxe',
  },
  {
    id: 'iron-armor',
    name: 'Iron Armor',
    type: 'equipment',
    description: 'Heavy, clunky, but it gets the job done.',
    pow: 0,
    vit: 10,
    spd: -2,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 5, silver: 0, iron: 3 },
    iconName: 'furCoat',
  },

  // --- Bronze Tier ---
  {
    id: 'bronze-sword',
    name: 'Bronze Sword',
    type: 'equipment',
    description: 'Better than iron, with a nice sheen.',
    pow: 8,
    vit: 3,
    spd: 0,
    forClass: 'warrior',
    comboBonus: 0.015,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 5, silver: 0, iron: 8 },
    iconName: 'copperSword',
  },
  {
    id: 'bronze-recurve-bow',
    name: 'Bronze Recurve Bow',
    type: 'equipment',
    description: 'A bow with a classic recurved shape, allowing for more power.',
    pow: 5,
    vit: 1,
    spd: 5,
    forClass: 'rogue',
    comboBonus: 0.02,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 4, silver: 0, iron: 6 },
    iconName: 'vineBow',
  },
  {
    id: 'bronze-staff',
    name: 'Bronze Staff',
    type: 'equipment',
    description: 'A bronze-capped staff that hums with a little power.',
    pow: 7,
    vit: 2,
    spd: 2,
    forClass: 'mage',
    comboBonus: 0.015,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 5, silver: 0, iron: 8 },
    iconName: 'rubyAxe',
  },
  {
    id: 'bronze-armor',
    name: 'Bronze Armor',
    type: 'equipment',
    description: 'Still heavy, but offers more protection than iron.',
    pow: 0,
    vit: 15,
    spd: -3,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 8, silver: 0, iron: 10 },
    iconName: 'paddedArmor',
  },

  // --- Silver Tier ---
  {
    id: 'silver-sword',
    name: 'Silver Sword',
    type: 'equipment',
    description: 'Lightweight and deadly. Especially against things that hate silver.',
    pow: 12,
    vit: 0,
    spd: 4,
    forClass: 'warrior',
    comboBonus: 0.02,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 0, silver: 6, iron: 4 },
    iconName: 'bronzeSword',
  },
  {
    id: 'silver-longbow',
    name: 'Silver Longbow',
    type: 'equipment',
    description: 'A tall bow crafted from ash and reinforced with silver.',
    pow: 8,
    vit: 0,
    spd: 8,
    forClass: 'rogue',
    comboBonus: 0.025,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 0, silver: 5, iron: 3 },
    iconName: 'crimsonBow',
  },
  {
    id: 'silver-staff',
    name: 'Silver Staff',
    type: 'equipment',
    description: 'Conducts magic with impressive efficiency.',
    pow: 11,
    vit: 2,
    spd: 5,
    forClass: 'mage',
    comboBonus: 0.02,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 0, silver: 7, iron: 4 },
    iconName: 'spikedMace',
  },
  {
    id: 'silver-armor',
    name: 'Silver Armor',
    type: 'equipment',
    description: 'Surprisingly tough for such a light metal.',
    pow: 0,
    vit: 22,
    spd: -1,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 0, silver: 10, iron: 5 },
    iconName: 'silverArmor',
  },

  // --- Steel Tier ---
  {
    id: 'steel-sword',
    name: 'Steel Sword',
    type: 'equipment',
    description: "The blacksmith's pride. Solid, reliable, and sharp.",
    pow: 16,
    vit: 4,
    spd: 1,
    forClass: 'warrior',
    comboBonus: 0.025,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 8, silver: 4, iron: 0 },
    iconName: 'steelSword',
  },
  {
    id: 'steel-composite-bow',
    name: 'Steel Composite Bow',
    type: 'equipment',
    description: 'A bow made of multiple materials for extreme tension.',
    pow: 10,
    vit: 1,
    spd: 10,
    forClass: 'rogue',
    comboBonus: 0.03,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 6, silver: 4, iron: 0 },
    iconName: 'roseBow',
  },
  {
    id: 'steel-staff',
    name: 'Steel Staff',
    type: 'equipment',
    description: 'Reinforced with steel. Surprisingly good for channeling magic.',
    pow: 15,
    vit: 2,
    spd: 4,
    forClass: 'mage',
    comboBonus: 0.025,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 8, silver: 5, iron: 0 },
    iconName: 'warAxe',
  },
  {
    id: 'steel-armor',
    name: 'Steel Armor',
    type: 'equipment',
    description: 'A solid upgrade. Much lighter than it looks.',
    pow: 0,
    vit: 30,
    spd: 0,
    cost: { coins: CRAFTING_FEE, gold: 0, copper: 10, silver: 6, iron: 0 },
    iconName: 'blueCoat',
  },

  // --- Gold Tier ---
  {
    id: 'golden-broadsword',
    name: 'Golden Broadsword',
    type: 'equipment',
    description: 'A blade of pure, enchanted gold. Heavy, but hits like a truck.',
    pow: 22,
    vit: 6,
    spd: -4,
    forClass: 'warrior',
    comboBonus: 0.03,
    cost: { coins: CRAFTING_FEE, gold: 4, copper: 0, silver: 6, iron: 0 },
    iconName: 'venomSword',
  },
  {
    id: 'golden-war-bow',
    name: 'Golden War Bow',
    type: 'equipment',
    description: 'A masterpiece of archery, gilded and enchanted for power.',
    pow: 14,
    vit: 2,
    spd: 12,
    forClass: 'rogue',
    comboBonus: 0.04,
    cost: { coins: CRAFTING_FEE, gold: 3, copper: 0, silver: 5, iron: 0 },
    iconName: 'fairyBow',
  },
  {
    id: 'golden-scepter',
    name: 'Golden Scepter',
    type: 'equipment',
    description: 'A symbol of magical authority that crackles with power.',
    pow: 20,
    vit: 5,
    spd: 6,
    forClass: 'mage',
    comboBonus: 0.03,
    cost: { coins: CRAFTING_FEE, gold: 4, copper: 0, silver: 6, iron: 0 },
    iconName: 'jadeAxe',
  },
  {
    id: 'golden-plate-armor',
    name: 'Golden Plate Armor',
    type: 'equipment',
    description: 'So heavy, but so protective. And so, so shiny.',
    pow: 0,
    vit: 40,
    spd: -5,
    cost: { coins: CRAFTING_FEE, gold: 6, copper: 0, silver: 8, iron: 0 },
    iconName: 'leatherRobe',
  },

  // --- Legendary & Joke Tier ---
  {
    id: 'gilded-steel-armor',
    name: 'Gilded Steel Armor',
    type: 'equipment',
    description: 'Masterwork steel armor with gold filigree. Peak protection.',
    pow: 5,
    vit: 45,
    spd: 0,
    cost: { coins: CRAFTING_FEE, gold: 8, copper: 0, silver: 12, iron: 0 },
    iconName: 'leatherRobe',
  },
  {
    id: 'gold-armor',
    name: 'Gold Armor',
    type: 'equipment',
    description: "Impressively shiny and incredibly soft. What, you thought it'd be good?",
    pow: 0,
    vit: 1,
    spd: -5,
    cost: { coins: CRAFTING_FEE, gold: 10, copper: 0, silver: 0, iron: 0 },
    iconName: 'leatherRobe',
  },
];

export const ConsumableItemIds = ['potion', 'high-potion', 'row-clear', 'column-clear', 'energy-potion'];
export const ConsumableItems: ConsumableItemData[] = [
  {
    id: 'potion',
    name: 'Potion',
    type: 'consumable',
    description: 'Heals 50 HP. Tastes like red.',
    cost: { coins: 50, gold: 0, copper: 0, silver: 0, iron: 0 },
    usableInBattle: true,
    usableOutOfBattle: true,
    action: { type: 'heal', amount: 50 },
    icon: '🧪',
    iconName: 'smallPotion',
  },
  {
    id: 'high-potion',
    name: 'High Potion',
    type: 'consumable',
    description: 'Heals 200 HP. Tastes like really red.',
    cost: { coins: 200, gold: 0, copper: 0, silver: 0, iron: 0 },
    usableInBattle: true,
    usableOutOfBattle: true,
    action: { type: 'heal', amount: 200 },
    icon: '⚗️',
    iconName: 'largePotion',
  },
  {
    id: 'row-clear',
    name: 'Row Clear',
    type: 'consumable',
    description: 'Wipe a row of your choice. Every orb counts and bombs go off.',
    cost: { coins: 300, gold: 0, copper: 0, silver: 0, iron: 0 },
    usableInBattle: true,
    usableOutOfBattle: false,
    action: { type: 'clear-line', orientation: 'row' },
    icon: '➡️',
    // Drawn by `RowClearIcon` (see `ItemIcon`): the sheet has no row/column glyph.
    iconName: null,
  },
  {
    id: 'column-clear',
    name: 'Column Clear',
    type: 'consumable',
    description: 'Wipe a column of your choice. Every orb counts and bombs go off.',
    // A column is 8 orbs to a row's 6, so it pays out ~33% more for ~33% more coin.
    cost: { coins: 400, gold: 0, copper: 0, silver: 0, iron: 0 },
    usableInBattle: true,
    usableOutOfBattle: false,
    action: { type: 'clear-line', orientation: 'column' },
    icon: '⬇️',
    // Drawn by `ColumnClearIcon` (see `ItemIcon`): the sheet has no row/column glyph.
    iconName: null,
  },
  {
    id: 'energy-potion',
    name: 'Energy Potion',
    type: 'consumable',
    description: 'A jolt for the whole team. Fills 30% of the ultimate bar.',
    cost: { coins: 400, gold: 0, copper: 0, silver: 0, iron: 0 },
    usableInBattle: true,
    usableOutOfBattle: false,
    action: { type: 'fill-ultimate', amount: 0.3 },
    icon: '⚡',
    iconName: 'largeBluePotion',
  },
];
