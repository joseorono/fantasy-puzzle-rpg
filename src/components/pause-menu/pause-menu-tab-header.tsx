...
interface PauseMenuTabHeaderProps {
  text: string;
  /** One-line description of what the player can do in this pane. Omit where the host already titles the pane. */
  hint?: string;
}
...
export function PauseMenuTabHeader({ text, hint }: PauseMenuTabHeaderProps) {
  return (
    <div className="pause-menu-tab-header">
      ...
      {hint && <p className="pause-menu-tab-header__hint pixel-font">{hint}</p>}
    </div>
  );
}
