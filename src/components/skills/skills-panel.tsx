...
import { SkillDetailPanel, type SkillSelection } from './skill-detail-panel';
import { SkillSlotTooltip } from './skill-slot-tooltip';
import { getUpgradePreviewRows } from './upgrade-preview';
import { describePassiveModifiers } from './passive-descriptions';
import { getDetailActions } from './skill-gates';
...
interface SkillsPanelProps {
  /** The host says when this pane owns the keyboard — arrows/Enter act on it. */
  keyboardActive?: boolean;
  /** Fired when ← from the roster hands the keyboard back to whatever nav the host owns. */
  onExitLeft?: () => void;
}

/**
 * Skill management, host-agnostic: pick a hero, browse their Active (Ultimate) track
 * and Passive track as framed Indigolay slots, inspect any slot in the parchment
 * detail panel, and unlock or level up skills with crafting resources — reaching a
 * level only makes a skill purchasable. Tier-0 Ultimates are the free starting kit.
 *
 * Brings no chrome of its own (no header, no frame), so any host can drop it in: the
 * pause menu's Skills tab today, the Training Grounds later.
 */
export function SkillsPanel({ keyboardActive = false, onExitLeft }: SkillsPanelProps) {
