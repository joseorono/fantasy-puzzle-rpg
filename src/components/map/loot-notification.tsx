...
export function LootNotification({ loot, onClose }: LootNotificationProps) {
  ...
  const hasResources =
    loot.resources.coins > 0 ||
    loot.resources.gold > 0 ||
    loot.resources.copper > 0 ||
    loot.resources.silver > 0 ||
    loot.resources.bronze > 0;
      ...
      <div className="space-y-2">
        ...
        {hasEquipment && (
          <div className="rounded bg-white/60 p-2">
            ...
            <ul className="space-y-0.5">
              {loot.equipableItems.map((item, idx) => (
        ...
        {hasConsumables && (
          <div className="rounded bg-white/60 p-2">
            ...
            <ul className="space-y-0.5">
              {loot.consumableItems.map((item, idx) => (
                <li key={idx} className="text-[11px] text-gray-700">
                  • {item.name}
                </li>
              ))}
        ...
        {hasResources && (
          <div className="rounded bg-white/60 p-2">
            ...
            <div className="mt-1 grid grid-cols-2 gap-1">
              {loot.resources.coins > 0 && (
                <span className="text-[11px] text-gray-700">Coins: {loot.resources.coins}</span>
              )}
              {loot.resources.gold > 0 && (
                <span className="text-[11px] text-gray-700">Gold: {loot.resources.gold}</span>
              )}
              {loot.resources.copper > 0 && (
                <span className="text-[11px] text-gray-700">Copper: {loot.resources.copper}</span>
              )}
              {loot.resources.silver > 0 && (
                <span className="text-[11px] text-gray-700">Silver: {loot.resources.silver}</span>
              )}
              {loot.resources.bronze > 0 && (
                <span className="text-[11px] text-gray-700">Bronze: {loot.resources.bronze}</span>
              )}
            </div>
          </div>
        )}
